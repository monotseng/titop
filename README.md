...
> [!CAUTION]
> **本工具的代码由 AI 辅助生成。** AI 生成的代码可能存在缺陷、不完整的兼容性处理或未被发现的安全风险。使用者应自行审查代码，并在非生产环境充分测试后，独立评估是否适合其系统和生产环境。运行 TiTop 所产生的性能、安全、数据及可用性风险由使用者自行承担。连接 TiDB 时请使用最小权限的专用只读监控账号，切勿使用 `root` 或其他高权限账号。
